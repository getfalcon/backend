...
class Menu extends Block
{
     ...
     */
    public function run() {
        return $this->renderNavigation($this->getMenuModel(), 0, 1);
    }
    ...
    public function renderNavigation($menu, $level = 0, $limit = 0, $colBrakes = []) {
        ...
        foreach ($this->_getMenuIterator($menu) as $menuItem) {
            ...
            $output .= Html::tag('li', $anchor . $subMenu, [
                'class' => ArrayHelper::merge(['item-' . $itemClass], $this->_renderItemCssClass($menuItem, $level)),
                'id' => $id,
                'role' => 'menu-item'
            ]);
            $itemPosition ++;
        ...
        return $outputStart . $output . Html::endTag('ul');
    }
